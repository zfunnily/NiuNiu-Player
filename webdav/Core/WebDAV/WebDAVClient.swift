...
class WebDAVClient {
    ...
    private func createRequest(for path: String, method: String, headers: [String: String]? = nil) -> URLRequest {
        ...
        DLog("创建请求: \(method) \(url.absoluteString)")

        var request = URLRequest(url: url)
        request.httpMethod = method
        DLog("Final URL:", request.url?.absoluteString ?? "nil")
        ...
        if let username = username, let password = password {
            ...
            let decodedString = String(data: decodedData, encoding: .utf8) {
                DLog("认证头解码验证: \(decodedString)")
            }
            // 打印认证头的长度（但不要打印完整内容，出于安全考虑）
            DLog("认证头设置，长度: \(authString.count)")
        } else {
            DLog("未提供认证信息")
        }
    ...
    func testConnection(completion: @escaping (Result<Bool, Error>) -> Void) {
        ...
        DLog("发送GET请求到: \(request.url?.absoluteString ?? "nil")")

        let task = session.dataTask(with: request) { data, response, error in
            if let error = error {
                DLog("连接错误: \(error.localizedDescription)")
                completion(.failure(error))
                return
            }
            
            if let httpResponse = response as? HTTPURLResponse {
                DLog("testConnection, 状态码: \(httpResponse.statusCode)")
                // 打印所有响应头以便调试
                DLog("响应头:")
